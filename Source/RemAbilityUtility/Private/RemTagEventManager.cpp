...
void FRemScopedAbilityTagEventManager::RegisterEvent(const FGameplayTag& Tag,
...
void FRemScopedAbilityTagEventManager::RegisterEventUnique(const FGameplayTag& Tag,
    const EGameplayTagEventType::Type EventType,
    const FEventDelegate& Delegate)
{
    if (!HasEvent(Tag))
...
void FRemScopedAbilityTagEventManager::UnRegisterEvents()
{
    if (IsValid())
    {
        for (const auto& Data : EventHandleMap)
        {
            ...
            if (bool bSucceed{AbilitySystem->UnregisterGameplayTagEvent(Data.Value, Data.Key,
                        EGameplayTagEventType::NewOrRemoved)
                };
                !bSucceed)
            {
                bSucceed = AbilitySystem->UnregisterGameplayTagEvent(Data.Value, Data.Key,
...
void FRemScopedAbilityGameplayEventManager::RegisterEvent(const FGameplayTag& Tag,
    const FEventDelegate& Delegate)
{
#if REM_WITH_DEVELOPMENT_ONLY_CODE
...
void FRemScopedAbilityGameplayEventManager::RegisterEventUnique(const FGameplayTag& Tag,
    const FEventDelegate& Delegate)
{
    if (!HasEvent(Tag))
    {
